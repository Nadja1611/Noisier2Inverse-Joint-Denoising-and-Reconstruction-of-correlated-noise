...
for sigma in [2, 3, 5]:
    ...
    # First subplot: PSNR Values
    psnr_data = [
        psnr_values_input, psnr_values_n2i,  # Input data and its variant
    ...
    psnr_labels = [
        'input', "N2I", "ours sobo z (psnr)", "ours sobo z ", "ours sobo y (psnr)", "ours sobo y", 
        "ours z(psnr)", "ours z", "ours y (psnr)", "ours y", "z (psnr)", "z", 
        "y (psnr)", "y",  
    ]
    colors_psnr = ['#D3D3D3', '#D3D3D3'] + ['#AEC6CF', '#FFB6C1'] * 6  # Gray for input and N2I
    ...
    handles = [
        ...
        plt.Line2D([0], [0], color='#D3D3D3', lw=4, label='input/N2I'),
    ]
    ...
    plt.tight_layout(pad=2.0)  # Fine-tune layout
    plt.savefig(
        os.path.join(
