...
for sigma in [2, 3, 5]:
    ...
    for f in files:
        ...
        if "Sob" in f and "sigma_" + str(sigma) in f:
            ...
            ssim_values_sob_z = np.load(os.path.join(path, f, "ssim_zpsnr_model_weights.pth.npy")
            )  # Changed ssim to ssim_values
            psnr_values_sob_z = np.load(os.path.join(path, f, "psnr_zpsnr_model_weights.pth.npy"))

            ssim_values_sob_y = np.load(os.path.join(path, f, "ssim_ypsnr_model_weights.pth.npy")
            )  # Changed ssim to ssim_values
    ...
    plt.savefig(
        os.path.join(
            ...
            "Plot_ssim_psnr_bestpsnr" + str(sigma) + ".png",
        )
    )
    ...
    if i == 0:
        fig, axes = plt.subplots(3, 6, figsize=(9, 5), constrained_layout=True)
        plt.subplots_adjust(wspace=0, hspace=0)  # Adjust spacing
    if i == 1:
        axes[i, 0].set_title("noisy ($\\sigma$ = " + str(3) + ")", fontsize=12)
    if i == 2:
        axes[i, 0].set_title("noisy ($\\sigma$ = " + str(5) + ")", fontsize=12)
    ...
    for j in range(6):  # 6 columns
        ...
        axins = inset_axes(
            ...
            borderpad=-0.4  # Adjust the distance between the main plot and the inset
        )
 ...
 #   plt.tight_layout()
    ...
    plt.savefig(
        os.path.join(
            args.outputdir + "/Plots_Paper",
            "bestpsnr_Plot_all" + "_y.svg",
        )
    )
    plt.savefig(
        os.path.join(
            args.outputdir + "/Plots_Paper",
            "bestpsnr_Plot_all" + "_y.png",
        )
    )
    i +=1
...
for sigma in [2, 3, 5]:
 ...
 #   plt.tight_layout()
    ...
    plt.savefig(
        os.path.join(
            args.outputdir + "/Plots_Paper",
            "bestpsnr_Plot_all" + "_z.svg",
        )
    )
    plt.savefig(
        os.path.join(
            args.outputdir + "/Plots_Paper",
            "bestpsnr_Plot_all" + "_z.png",
        )
    )
    i +=1
